	...
	<div>
		...
		<div class="mb-6 grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
			<div v-for="stat in stats" :key="stat.label" class="rounded-2xl border border-gray-200 bg-white p-5">
				...
				<div class="mt-2 flex items-center gap-1 text-sm text-gray-500">
					...
					<span :class="stat.hasTrendIcon ? 'text-green-600' : 'text-gray-500'">{{ stat.trend }}</span>
				</div>
			</div>
		</div>
					...
					<div v-for="event in upcomingEvents" :key="event.name">
						<div class="mb-1.5 flex items-start justify-between">
							...
							<span class="shrink-0 rounded-full px-2 py-0.5 text-xs font-semibold"
								:class="event.progress >= 80 ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-700'">
								{{ event.progress }}%
							</span>
						</div>
						<div class="mb-2 text-xs text-gray-400">
							{{ event.date }} &bull; {{ event.guests }} guests</div>
						<div class="mb-1 flex items-center justify-between text-xs text-gray-400">
							<span>Preparation Progress</span>
							<span>{{ event.progress }}%</span>
						</div>
						<div class="h-2 w-full overflow-hidden rounded-full bg-gray-100">
							<div class="h-full rounded-full bg-gray-900" :style="{ width: event.progress + '%' }" />
						</div>
					</div>
				</div>
			</div>
		...
		<div class="rounded-2xl border border-blue-100 bg-blue-50/60 p-6">
			...
			<p class="text-sm text-gray-600">
				All systems operational. You're receiving real-time notifications for matching inquiries based on your
				service tags and location preferences.
			</p>
		</div>
	</div>
</template>
